...
import {Circle, CircleOptions, GoogleMap, MapsEventListener, Marker, MarkerOptions, MVCObject, Polygon, PolygonOptions, Polyline, PolylineOptions, Rectangle, RectangleOptions} from '@lvmaps/core/services/google-maps-types';
...
export class AgmDrawingManager implements OnChanges, OnDestroy {
  ...
  setMap(map: GoogleMap) {
    if (!google.maps.drawing && isDevMode()) {
      console.error(
          'Cannot use drawing manager if drawing library is not ' +
  ...
  initEvents(drawingManager: any) {
    ...
    this.eventSubscriptions.push(
        this.createMvcObservable<OverlayCompleteEvent>('overlaycomplete', drawingManager)
            .subscribe(
                overlayevent => this._zone.run(() => this.overlayComplete.next(overlayevent))));
    this.eventSubscriptions.push(
  ...
  createMvcObservable<E>(eventName: string, mvcObject: MVCObject): Observable<E> {
    return fromEventPattern(
        handler => mvcObject.addListener(eventName, (event?: E) => handler.apply(null, [event])),
